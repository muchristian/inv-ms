...
const Stock = () => {
  ...
  const { data: stock, isFetching } = useGetStockQuery({
    date: currDate,
    searchQuery,
    ...pagination,
  });
  const [stockCreate] = useAddStockMutation();
  const [stockUpdate] = useUpdateStockMutation();
    ...
    const inputNode =
      inputType === "number" ? (
        <InputNumber
          ...
          disabled={["consumed"].includes(dataIndex) && true}
        />
      ) : (
        <Input />
      );
  ...
  const save = async (record: Item, mode: string) => {
    try {
      if (mode !== "add") {
        ...
        const data = {
          ...omit(record, ["productName", "productPrice"]),
          ...row,
        };
        const result = await stockUpdate(data).unwrap();
        form.resetFields();
        cancel();
        ...
        const row = (await form.validateFields()) as Item;
        const data = {
          ...omit(record, ["productName", "productPrice", "_id"]),
          ...row,
          date: currDate,
        };
        const result = await stockCreate(data).unwrap();
        form.resetFields();
        cancel();
        const { message } = result;
        ToastRender(message);
  ...
  const onExportCsV = () => {
    axios
      .get(
        `http://localhost:8001/api/inventory/stock/export?date=${currDate}`,
        {
          responseType: "arraybuffer",
        }
      )
      .then((e) => {
        fileDownload(e.data, `stock-report-${currDate}.xlsx`);
      })
      .catch((err) => {
        ToastRender(err);
      });
          ...
          <div className="flex flex-col bg-primary p-4 gap-4 rounded border border-text/10">
            ...
            <Form form={form} component={false}>
              <Table
                ...
                // }
                dataSource={stock ? stock.data.items : undefined}
                columns={mergedColumns}
                rowClassName="editable-row"
                rowKey="productName"
                pagination={{
                  ...pagination,
                  total: stock ? stock.data.total : 0,
                }}
